...
const OrderTable = ({
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 px-4">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">
          No {orderType} coating orders found
        </h3>
        <p className="text-sm text-gray-500 text-center max-w-sm">
          {orderType === 'pending'
            ? 'When you receive new orders, they will appear here for easy management and tracking.'
            : 'Completed orders will appear here once all coating in an order is finished.'}
        </p>
      </div>
    );
  }
  ...
  const isEditEnabled = (coating) => {
    if (!coating) return false;
    return coating.is_partially_received &&
      coating.is_completely_received &&
      coating.is_deco_no_approved;
  };
      ...
      <div className="hidden xl:block">
        ...
        {currentOrders.map((order, orderIndex) => {
          ...
          order.items?.forEach(item => {
            ...
            totalOrderRows += Math.max(1, coatings.length);
          });
          ...
          return (
            <div key={`order-${order.order_number}`} className="bg-white rounded-lg shadow-sm border border-orange-200 mb-3 overflow-hidden">
              {order.items?.map((item, itemIndex) => {
                const coatings = item.coating || [];
                const coatingsToShow = coatings.length === 0 ? [null] : coatings;
                const bgColor = colorClasses[itemIndex % colorClasses.length];

                return coatingsToShow.map((coating, coatingIndex) => {
                  const isFirstRowOfOrder = currentRowInOrder === 0;
                  const isFirstRowOfItem = coatingIndex === 0;
                  const isLastRowOfOrder = currentRowInOrder === totalOrderRows - 1;
                  currentRowInOrder++;

                  const remainingQty = coating ? getRemainingQty(coating) : 'N/A';
                  const status = coating ? coating.status : 'N/A';

                  return (
                    <div
                      key={`${order.order_number}-${item.item_name}-${coating?.coating_id || 'empty'}-${coatingIndex}`}
                    ...
                    >
                      ...
                      <div className="text-left text-orange-900 col-span-3">
                        <div className="flex items-center gap-1">
                          <span>{coating ? (coating.bottle_name || 'N/A') : 'N/A'}</span>
                          {coating && coating.bottle_name && (
                            <button
                              onClick={() => handleCopyCoatingName(coating.bottle_name)}
                              className="p-0.5 text-orange-600 hover:text-orange-800 hover:bg-orange-50 rounded transition-colors"
                              title="Copy coating type to search"
                            >
                      ...
                      <div className="text-left text-red-900 font-semibold flex justify-center">
                        {coating ? (coating.available_stock || 0) : 'N/A'}
                      </div>
                      <div className="text-left text-red-900 font-semibold flex justify-center">
                        {coating ? (coating.inventory_used || 0) : 'N/A'}
                      </div>
                      ...
                      <div className="text-center">
                        {coating ? (
                          <input
                            type="checkbox"
                            checked={coating.is_partially_received || false}
                            onChange={(e) => handlePartiallyReceivedChange(coating.coating_id, e.target.checked)}
                      ...
                      <div className="text-center">
                        {coating ? (
                          <input
                            type="checkbox"
                            checked={coating.is_completely_received || false}
                            onChange={(e) => handleCompletelyReceivedChange(coating.coating_id, e.target.checked)}
                      ...
                      <div className="text-center text-orange-900">
                        {coating ? (coating.deco_no || 'N/A') : 'N/A'}
                      </div>
                      ...
                      <div className="text-center col-span-2">
                        {coating ? (
                          <span className={`px-2 py-1 rounded text-xs font-medium ${coating.is_deco_no_approved
                            ? 'text-green-800'
                            : 'text-orange-800'
                            }`}>
                            {coating.is_deco_no_approved ? 'Approved' : 'Awaiting '}
                          </span>
                        ) : (
                          <span className="text-gray-400">-</span>
                        )}
                      ...
                      <div className="text-center">
                        {isFirstRowOfItem && orderType === 'pending' ? (
                          <button
                            ...
                            disabled={!isEditEnabled(coating)}
                            className={`inline-flex items-center justify-center p-1.5 rounded text-white transition-colors duration-200 shadow-sm ${isEditEnabled(coating)
                              ? 'bg-orange-600 hover:bg-orange-500 cursor-pointer'
                              : 'bg-gray-400 cursor-not-allowed'
                              }`}
                            title={isEditEnabled(coating) ? "Edit quantities" : "Complete all requirements to enable editing"}
                          >
                            <Edit size={14} />
                          </button>
                        ) : (
                    ...
                    <div className="px-4 py-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {orderType === 'pending' && (
                          <button
                            onClick={() => handleEditClick(order, item)}
                            disabled={coatings.length > 0 && !isEditEnabled(coatings[0])}
                            className={`p-2 rounded text-white transition-colors ${coatings.length === 0 || isEditEnabled(coatings[0])
                      ...
                      {coatings.length > 0 ? (
                        <div className="space-y-3">
                          {coatings.map((coating, coatingIndex) => {
                            const remainingQty = getRemainingQty(coating);
                            const status = coating.status;
                            const rowId = `${order.order_number}-${item.item_name}-${coating.coating_id}`;
                            const isExpanded = expandedRows.has(rowId);

                            return (
                              <div key={coating.coating_id} className="bg-white rounded-lg p-3 shadow-sm">
                                <div className="flex items-center justify-between mb-2">
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-1">
                                      ...
                                      <button
                                        ...
                                        title="Copy coating info to search"
                                      >
                                        <svg className="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                                        </svg>
                                    ...
                                    <div className="flex items-center gap-4 mt-1">
                                      <span className="text-xs text-gray-600">
                                        Qty: <span className="font-medium text-orange-900">{coating.quantity}</span>
                                      </span>
                                      <span className="text-xs text-gray-600">
                                ...
                                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                                  ...
                                  <span className={`px-2 py-1 rounded text-xs font-medium ${coating.is_deco_no_approved
                                    ...
                                    {coating.is_deco_no_approved ? 'Approved' : 'Waiting'}
                                  </span>
                                </div>
                                  ...
                                  <div className="pt-2 border-t border-gray-100 mt-3">
                                    <div className="grid grid-cols-2 gap-3 text-xs">
                                      <div>
                                        ...
                                        <p className="font-medium text-gray-900">{coating.design_name || 'No Design'}</p>
                                      </div>
                                      <div>
                                        <span className="text-gray-500">Color:</span>
                                        <p className="font-medium text-gray-900">{coating.color || 'No Color'}</p>
                                      </div>
                                      <div>
                                        <span className="text-gray-500">Coating ID:</span>
                                        <p className="font-medium text-gray-900">{coating.coating_id || 'N/A'}</p>
                                      </div>
                                      <div>
                                        <span className="text-gray-500">Rate:</span>
                                        <p className="font-medium text-gray-900">₹{coating.rate || 'N/A'}</p>
                                      </div>
                                      <div>
                                        <span className="text-gray-500">Available Stock:</span>
                                        <p className="font-medium text-gray-900">{coating.available_stock || 0}</p>
                                      </div>
                                      <div>
                                        <span className="text-gray-500">Completed:</span>
                                        <p className="font-medium text-gray-900">{coating.completed_qty || 0}</p>
                                      </div>
                                    </div>
                                  </div>
                                )}
