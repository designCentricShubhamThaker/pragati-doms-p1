...
const VehicleDetails = ({ isOpen, onClose, orderData, itemData, componentId, onUpdate }) => {
  ...
  useEffect(() => {
    ...
    const handleVehicleUpdated = ({ order_number, item_id, component_id, updatedComponent }) => {
      ...
      // Update local state instead of relying on parent re-render
      ...
      // Call parent onUpdate without closing modal
      if (onUpdate) {
  ...
  }, [socket, localItemData, orderData, onUpdate]);
  ...
  const handleMarkAllAsDelivered = async (componentId) => {
    ...
    try {
      // Mark all vehicles as DELIVERED
      ...
      console.log('Emitting updateGlassVehicle for mark all as delivered:', payload);
      socket.emit("updateGlassVehicle", payload);
  ...
  const handleMarkAllAsApproved = async (componentId) => {
    ...
    try {
      // Mark all delivered vehicles as approved
      const updatedVehicleDetails = component.vehicle_details.map(vehicle => ({
        ...vehicle,
      ...
      console.log('Emitting updateGlassVehicle for mark all as approved:', payload);
      socket.emit("updateGlassVehicle", payload);
...
const componentsWithVehicles = componentId 
  ? localItemData.components?.filter(comp => comp.component_id === componentId) || []
  : localItemData.components?.filter(
      component => component.vehicle_details && component.vehicle_details.length > 0
    ) || [];

// If specific component ID provided but component has no vehicle details
if (componentId && componentsWithVehicles.length > 0 && 
    (!componentsWithVehicles[0].vehicle_details || componentsWithVehicles[0].vehicle_details.length === 0)) {
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 ${isOpen ? '' : 'hidden'}`}>
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 text-center">
          <p className="text-gray-500">No vehicle details found for {componentsWithVehicles[0]?.name || 'this component'}.</p>
        </div>
      </div>
    </div>
  );
...
if (componentsWithVehicles.length === 0) {
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 ${isOpen ? '' : 'hidden'}`}>
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
        ...
        <div className="p-6 text-center">
          <p className="text-gray-500">No vehicle details found for this component.</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-gray-900">
              Vehicle Details - {localItemData.item_name}
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4">
          ...
          {componentsWithVehicles.length > 0 ? (
            componentsWithVehicles.some(comp => comp.vehicle_details && comp.vehicle_details.length > 0) ? (
              <div className="space-y-6">
                {componentsWithVehicles.map((component, componentIndex) => {
                  // If no vehicle details for this specific component, show message
                    ...
                    return (
                      <div key={component.component_id} className="border border-gray-200 rounded-lg p-4">
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-gray-500">No vehicle details available for this component</p>
                        </div>
                      </div>
                    );
                  }
              ...
              <div className="text-center py-8">
                <Truck className="w-12 h-12 text-gray-300 mx-auto mb-3" />
                <p className="text-gray-500">No vehicle details available for this {componentId ? 'component' : 'item'}</p>
              </div>
            )
          ) : (
            <div className="text-center py-8">
